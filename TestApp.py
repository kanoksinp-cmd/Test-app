...
# รายชื่อธนาคารหลักและ URL Scheme สำหรับเปิดแอปบนมือถือ
BANK_CONFIG = {
    "กสิกรไทย (KBank)": {"name": "กสิกรไทย (KBank)", "scheme": "kplus://"},
    "ไทยพาณิชย์ (SCB)": {"name": "ไทยพาณิชย์ (SCB)", "scheme": "scbeasy://"},
    "กรุงไทย (KTB)": {"name": "กรุงไทย (KTB)", "scheme": "krungthaibanking://"},
    "กรุงเทพ (BBL)": {"name": "กรุงเทพ (BBL)", "scheme": "bualuangmbanking://"},
    "กรุงศรีอยุธยา (BAY)": {"name": "กรุงศรีอยุธยา (BAY)", "scheme": "kma://"},
    "ทหารไทยธนชาต (TTB)": {"name": "ทหารไทยธนชาต (TTB)", "scheme": "ttbtouch://"},
    "ออมสิน (GSB)": {"name": "ออมสิน (GSB)", "scheme": "gsbmyamo://"},
    "ยูโอบี (UOB)": {"name": "ยูโอบี (UOB)", "scheme": "uobmightyth://"}
}
...
with tab3:
    ...
    if not expenses_rows: st.info("ยังไม่มีข้อมูล")
        ...
        st.subheader("🚀 แผนการโอนเงิน (พร้อมข้อมูลบัญชี & ปุ่มเปิดแอป)")
        ...
        while debtors and creditors:
            ...
            pp_info = user_profiles.get(creditor_name, {}).get("promptpay", "")
            b_name = user_profiles.get(creditor_name, {}).get("bank_name", "")
            b_acc = user_profiles.get(creditor_name, {}).get("bank_acc", "")
            
            account_details = []
            if pp_info: account_details.append(f"📱 พร้อมเพย์: {pp_info}")
            if b_name and b_acc: account_details.append(f"🏦 ธนาคาร: {b_name} เลขบัญชี: {b_acc}")
            elif b_acc: account_details.append(f"🏦 เลขบัญชี: {b_acc}")
            
            account_str = " | ".join(account_details) if account_details else "❌ ยังไม่ได้บันทึกข้อมูลบัญชี"
            
            # --- สร้างกล่องแจ้งโอนเงิน ---
            st.info(f"💳 **{debtor_name}** โอนให้ 👉 **{creditor_name}** จำนวน **{amt:,.2f}** บาท\n\n 📋 *ช่องทางโอนเงินของผู้รับ:* {account_str}")
            
            # --- สร้างปุ่ม Deep Link เพื่อเปิดแอปพลิเคชันมือถือ ---
            if b_name in BANK_CONFIG:
                scheme_url = BANK_CONFIG[b_name]["scheme"]
                # ใช้ HTML/Markdown ช่วยทำปุ่ม Link สวยๆ
                st.markdown(f'''
                    <a href="{scheme_url}" target="_blank" style="text-decoration: none;">
                        <div style="display: inline-block; padding: 6px 14px; background-color: #2e7d32; color: white; border-radius: 4px; font-weight: bold; font-size: 14px; margin-bottom: 15px;">
                            📲 กดเปิดแอป {b_name} เพื่อโอนเงิน
                        </div>
                    </a>
                ''', unsafe_allow_whitespace=False, unsafe_allow_html=True)
            elif pp_info:
                # กรณีมีแต่พร้อมเพย์ แนะนำเปิด K PLUS เป็นค่าเริ่มต้นทั่วไปได้
                st.markdown('''
                    <a href="kplus://" target="_blank" style="text-decoration: none;">
                        <div style="display: inline-block; padding: 6px 14px; background-color: #1976d2; color: white; border-radius: 4px; font-weight: bold; font-size: 14px; margin-bottom: 15px;">
                            📱 กดเปิดแอปธนาคาร (K PLUS) โอนพร้อมเพย์
                        </div>
                    </a>
